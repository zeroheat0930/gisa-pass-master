...
> **2026-08-10 재점검:** Fable 5 전체 재감사에서 **미검증 발견 26건 + 직접 발견 5건**이 새로 나왔다.
> 정본은 **`docs/FABLE5_AUDIT_2026-08-10.md`** — 수정 작업은 그 문서의 우선순위 순서로 진행할 것.
> 특히 **PrivacyInfo.xcprivacy가 Xcode 프로젝트에 미등록**(직접 확인됨)이라 아래 문서의
> 프라이버시 매니페스트 수정이 아직 앱 번들에 실리지 않는다.
...
- [x] ~~프리미엄 게이팅 사실상 무력화 (1일/3일 플랜 전 구간 무료, 플랜 리셋 무제한)~~
      ...
      v1.8.2+40 빌드에 포함 — 39 는 업로드 전 폐기하고 40 으로 대체 (커밋 `7f93de0`)
