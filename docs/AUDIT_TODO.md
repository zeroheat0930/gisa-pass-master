...
- 이 문서 작성 시점 기준 **수정 완료 12건, 남은 확정 약 35건**
...
- [ ] `lib/screens/past_exam_screen.dart:436` — **문제은행 탭·AI 예측 모의고사 풀이가 DB에 전혀
      기록되지 않음.** 통계·오답노트·스트릭·AI예측이 모두 반영 안 됨. 메인 탭이라 영향 최대
...
- [ ] `lib/services/database_service.dart:107` — 문제 시드가 모바일/웹 두 벌로 갈라져 있고, 모바일은
      최초 설치 때 1회만 읽어 **정답 수정이 기존 유저에게 반영되지 않음** (아래 데이터 수정과 직결)
...
코드를 직접 계산해 정정값까지 확인함. **적용만 하면 된다.**

| 파일 | 인덱스(0-based) | 현재 정답 | 정정값 |
|---|---|---|---|
...
> **주의:** 1차 감사가 지목한 `c_questions.json` 22번(오타 `threefoureother`)은 인덱스 21을 직접
> 확인한 결과 정답이 `4 6`으로 **정상이었다.** 다른 인덱스를 가리킨 것으로 보이므로 해당 문자열을
> 전체 검색해 위치를 다시 찾을 것. 못 찾으면 오탐으로 처리.

> **중요:** 데이터를 고쳐도 `database_service.dart:107` 때문에 **기존 유저에게는 반영되지 않는다.**
> 문제 시드 갱신 마이그레이션을 같이 넣어야 의미가 있다.
...
- [ ] `lib/services/database_service.dart` — questions 테이블 갱신 마이그레이션 부재로 정답 수정이
      기존 유저에게 영원히 반영 안 됨 *(위 데이터 수정과 직결되므로 사실상 확인 필요)*
