...
> **2026-08-11 수정 반영 현황 (v1.6.1+25):**
> - **수정 완료:** #1(Android 지문 검사 생략) · #2(복원 가용성 재확인+결과 피드백) ·
>   #3(자동 복원 시도 상한 3회) · #4(purchaseStream 구독을 관리자 분기 앞으로) ·
>   #5(_retry 재진입 가드+_adCounter 리셋) · #7(광고 준비 라벨 폴링 갱신) ·
>   #8('예정' 표시 + 추정 일정 D-Day 알림 미발송) · #9(추정이 확정 회차 재표시 안 함) ·
>   #10(부분 실패 시 예외 전파, 리비전 미기록) · #11(DB getter Future 메모이제이션) ·
>   #13·16(시딩/v6 완료 시 리비전 기록) · #14(PrivacyInfo pbxproj 등록) ·
>   #18~20(채점 3건) · #22(리워드 지급 배선 테스트, 뮤테이션 2종 검증) ·
>   #24(통계 재조회 배선 테스트) · #26(grep 이 주석 제외) · D1 · D3 · D4 · D5
> - **문서화로 대응:** #12 — 본문 수정 금지·마이그레이션 절차를 syncQuestionsFromAssets 주석에 명시
> - **미수정 잔여:** #6(사람: AdMob 콘솔에서 리워드 단위 생성 후 ID 교체) ·
>   #21(설계 판단 필요: code_reading 대소문자 전면 접기) ·
>   #23(회차별 문제집 테스트 0건) · #25(app_test 연타 테스트 공회전)
