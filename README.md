...
3. **(1의 변종) 복붙된 흐름을 정본으로 뽑았으면 배선 테스트는 두 방향을 본다.**
   "정본에 핵심 로직이 있다"만 단언하면 화면이 사본을 다시 적어도 통과한다.
   `wiring_test.dart` 의 AI 모의고사 진입 검사처럼 **"화면은 정본을 부르고,
   핵심 호출을 직접 갖지 않는다"** 까지 단언할 것.
...
문항은 **출처가 두 갈래**이고, 앱은 이를 절대 섞지 않는다.

| 구분 | 파일 | 정체 | 표기 |
|------|------|------|------|
| **복원 기출** | `restored_exam_questions.json` | 실제 시험을 응시자들이 복원한 문제 | "복원 기출" + 출처 표시 |
| **AI 예상문제** | 나머지 5개 파일 (1,000문항) | AI 가 만든 예상문제 | "AI 예상" |

`questions.source` 컬럼(DB v8)이 이 구분을 담는다. 화면은 **이 값만 보고** 표기한다 —
연도로 추측하면 안 된다(그렇게 했다가 AI 문항이 '기출'로 표시된 적이 있다).

### 복원 기출의 출처와 라이선스

출처는 **Life-Journey 블로그**(<https://chobopark.tistory.com>)의 회차별 복원문제 글이다.
이 블로그는 각 글에 **CC BY 4.0**(저작자표시) 라이선스를 걸어두었다.

- 확인 방법: 글 하단 `♡ 공감`·`구독하기` 버튼 줄 오른쪽 끝에 CC 저작자표시(BY)
  아이콘이 **실제로 렌더링된다**(작아서 놓치기 쉬울 뿐 숨겨진 것이 아니다).
  HTML 소스에는 `creativecommons.org/licenses/by/4.0/deed.ko` 링크와 티스토리
  CCL 마크업(`ccl-type`, `ccl-derive`)이 들어 있다.
- **티스토리 정책이 아니라 저작자가 직접 켠 설정이다.** 블로그마다 값이 다르다 —
  chobopark 은 `ccl-type=2`(CC BY), hbase.tistory.com 은 `ccl-type=5`(CC BY-NC-SA).
  플랫폼 강제라면 모두 같아야 한다. 게다가 CC BY 는 비영리(NC)·변경금지(ND)를
  **붙이지 않은** 가장 열린 선택지다.
- CC BY 4.0 이 허용: 복제·배포, 변형·2차적 저작물, **상업적 이용**(광고·유료 앱 포함).
  그리고 **조건을 지키는 한 취소되지 않는다**(제2조 (a)(1), `irrevocable`).
  저작자가 나중에 CCL 설정을 꺼도 이미 받은 이용 권한은 소멸하지 않는다.
- 증거는 `docs/license-evidence/` 에 있다 — 원문 HTML·SHA-256 해시·
  web.archive.org 스냅샷 타임스탬프·배지 스크린샷.

**CC BY 4.0 이 요구하는 것은 네 가지다**(제3조 (a)(1)). 하나라도 빠지면 라이선스가
소멸하고(제6조 (a)) 무단 이용이 된다.

| 의무 | 앱에서 어디에 |
|---|---|
| 저작자 | `Life-Journey` |
| 라이선스 종류 | `CC BY 4.0` |
| 라이선스 링크(URI) | `creativecommons.org/licenses/by/4.0` |
| 변경 사실 공지 | `원문의 이미지·표를 … 텍스트로 옮겼습니다` |

넷 다 **두 곳**에 있다.

1. 복원 기출 회차 목록 상단 고지 (`lib/screens/round_list_screen.dart` 의 `_notice()`)
2. 라이선스 목록 화면 (홈 개발자 카드 → `문항 출처 · 오픈소스 라이선스`).
   `LicenseNotices.register()` 가 등록한다 — 패키지가 아닌 저작물은 Flutter 가
   자동으로 모아주지 않기 때문이다.

`test/round_list_screen_test.dart` 와 `test/license_notice_test.dart` 가
넷 중 하나라도 빠지면 실패한다.

### 오픈소스 라이선스 고지

pub 패키지는 대부분 BSD·MIT·Apache 2.0 이고 셋 다 **배포물에 저작권 고지와
라이선스 사본을 포함할 것**을 조건으로 건다. 홈의 개발자 카드에 있는
`문항 출처 · 오픈소스 라이선스` 줄이 `showLicensePage` 를 연다. Flutter 가 빌드
때 모아둔 패키지 라이선스가 그대로 나오므로 이 입구 하나면 충족된다.
**이 입구를 지우면 조건을 안 지킨 것이 된다.**

**정책상 지킬 것 두 가지.**
1. **복원 기출은 유료 잠금 없이 전체 공개한다.** 남에게 받은 자료를 유료 상품으로
   파는 것은 맞지 않는다. (광고는 앱 전체와 동일하게 노출되며, 광고 제거는 프리미엄의
   몫이다 — 출처 블로그도 광고로 운영된다.)
2. **저작자 표시를 지우거나 "직접 복원했다"고 쓰지 않는다.** 표시가 이 라이선스의
   조건이라, 그것만 어겨도 라이선스가 소멸한다.
3. **"형식을 바꿨으니 우리 것"은 성립하지 않는다.** 원문 이미지를 읽어 JSON 으로
   옮긴 것은 복제·개작이지 새 저작물의 창작이 아니다. 저작권은 표현에 붙지 파일
   형식에 붙지 않는다. 특히 **해설 문장**은 블로그 운영자가 직접 쓴 것이다.
   다행히 CC BY 는 개작을 명시적으로 허용하므로, 표시만 유지하면 지금 형태
   그대로 상업적으로 써도 된다.

> 남는 회색지대: 블로그 주인이 라이선스를 걸 수 있는 것은 **본인 기여분**(수집·정리·
> 해설)까지다. 원 시험문제의 권리는 한국산업인력공단에 있다. 이는 시중 기출문제집과
> 복원문제 블로그가 공통으로 안고 있는 영역이다.

### AI 예상문제

`assets/questions/*.json` 의 나머지 **1,000문항**.
...
### v1.8.3+41 (2026-09-22) — 3회차 전환 퍼널판

**접수(9/21)가 이미 시작돼 다운로드 피크가 진행 중이다.** 기능을 크게 넣는 대신
"이미 있는 유료 입구를 보이게 하는" 것만 묶어 작게 빨리 내보낸다. 150분 타이머
모의고사는 +42(시험 후)로 미룬다 — 복원 기출은 무료라 직접 수익이 없고, 세
진입 경로가 공유하는 퀴즈 화면을 건드려 회귀 면적이 가장 넓기 때문.
계획 정본과 합의 기록은 `.omc/plans/round3-update-plan.md`.

#### 전환 퍼널
- **복원 기출 결과 화면에 유료 퍼널이 0건이었다.** 무료로 열린 420문항이 가장 큰
  트래픽인데 출구가 막혀 있었다. "AI 실전 모의고사로 실력 점검" / "광고 없이
  공부에 집중"(프리미엄이면 숨김) 두 CTA 를 단다
- AI 모의고사 진입(쿼터 게이트 → 소진 다이얼로그 → 로드 → push)은
  `services/ai_exam_launcher.dart` 의 `startAiExam` **정본 하나**를 홈과 결과 화면이
  같이 쓴다. 배선 테스트가 "정본에 게이트가 있고, 화면은 정본만 부른다"를
  지킨다 — 한쪽 입구만 쿼터를 안 물으면 무료 하루 1회가 통째로 무의미해진다
- **홈 첫 화면 재배치** — 유료 입구(AI 모의고사·학습 플랜)가 정보 카드 뒤 5~6번째라
  iPhone 874 높이에서 41px 밖이었다. D-Day 바로 아래로 올린다. AI 버튼 제목이
  iPhone SE 기본 배율에서도 14px 가로 오버플로였던 것(기존 결함)을 함께 고침
- **카피는 사실 서술만.** 쿼터 소진 다이얼로그·구독 화면에 "무료는 하루 1회 ·
  시험까지 D-26" 한 줄. 숫자는 게이팅 정본에서 읽고, 노출은
  `AppConfig.shouldShowExamCountdown`(확정 일정 && 60일 이내)이 혼자 판단한다 —
  시험일이 지나면 다음 회차 D-181 이 되어 헛소리가 되기 때문. 가격은
  `utils/price_format.dart` 의 `formatPrice` 정본(`premiumPrice` 가 정의만 되고
  화면엔 `'₩4,900'` 하드코딩이었다). 기능표는 ○/× 대신 실제 횟수
- 결과 화면에 "실전 환산: N문항 기준 M분 중 ○○분 사용" 한 줄(문항 수 비례,
  기준 초과 시 "초과"로만 표기)

#### 인앱 리뷰 요청 (신규 의존성 `in_app_review`)
- 평생권이라 재구매가 없다 — 리뷰·랭킹이 유일한 증폭 수단인데 요청 API 가
  없었다. 복원 기출 결과 화면 1곳에서, 세션 정답률 ≥60% + 누적 30문항(도입 이후
  기준) + 90일 쿨다운일 때만. CTA 를 눌렀거나 쿼터 다이얼로그가 떴으면 그
  세션엔 묻지 않는다(다이얼로그 겹침 방지)
- `in_app_review`·전이 의존성 `url_launcher_ios` 의 `PrivacyInfo.xcprivacy`
  동봉·tracking=false 확인. `test/privacy_manifest_test.dart` 가 플러그인
  매니페스트까지 검사한다(빌드 37·38 이 반려된 ITMS-91064 재발 방지)
- Play 리뷰 다이얼로그는 **Play 설치본에서만** 동작한다(사이드로드는 no-op).
  내부 테스트 트랙으로 확인할 것

#### 정리
- `lastRealExamYear`/`isPredictedYear`/`latestQuestionYear` 삭제 — lib 에서
  참조가 없고 실제 기출/예상 구분은 `question.isRestored` 가 한다

#### 검증
- 테스트 310건 → **379건** (`analyze` 0건). 신규 테스트는 전부 "변경을 되돌리면
  실제로 실패하는가"를 실측해 확인했다. 독립 코드 리뷰에서 "지키는 코드는
  있는데 되돌려도 통과하는 테스트" 2건(리뷰·CTA 상호배제, 세션 내 1회 가드)이
  잡혀 시임을 넣어 진짜 검증으로 바꿨다
- 성공 판정은 10/1~10/18 스토어 콘솔의 다운로드 대비 구매 건수(직전 3.7%) 하나.
  애널리틱스가 없어 항목별 귀인은 불가능하다. 리뷰 수 증감만 리뷰 요청 단독
  지표로 본다

### v1.8.2+40 (2026-09-07)

**9/21 3회차 접수 시작 전 전면 재점검판.** 빌드 39(8/14, 업로드 전)에 재점검에서
나온 수익 결함 수정을 얹어 빌드 40 으로 한 번에 간다. 39 는 폐기.

재점검은 두 갈래로 돌렸다 — ① 릴리스 준비 검증(빌드 산출물을 IPA 내부까지
전수 확인: 프라이버시 매니페스트 tracking=false, versionCode, 복원 기출
21회차 420문항), ② 결제·광고·게이팅 코드 리뷰. ①은 통과, ②에서 확정 결함이
나와 전부 수정했다.

#### 수익 게이팅 (감사 잔여분 해소)
- **1일/3일 플랜이 전 구간 무료 + 플랜 초기화 무제한**이라, 짧은 플랜만 리셋해
  가며 쓰면 유료 구간(Day 4+)을 영영 살 이유가 없었다.
  무료 규칙을 "짧은 플랜(1·3일)은 Day 1, 긴 플랜(5·7·14일)은 Day 3까지"로 통일하고,
  무료 유저의 초기화를 하루 1회로 제한(`PlanResetQuota`, AiExamQuota 와 같은 방식).
  게이팅 판정은 `StudyPlanService.needsPremiumForDay` 정본으로 이동
- 1일/3일 플랜은 스토어에 나간 적이 없어(라이브는 v1.5.3) 소급으로 기능을
  잃는 유저는 없다

#### 결제·광고 (리뷰 확정 5건)
- **결제 실패·취소 트랜잭션 미종결** — StoreKit 은 error/canceled 도
  `pendingCompletePurchase` 로 내려주는데 종결하지 않아, 큐에 남아 앱을 켤
  때마다 재전달되고 취소 유저마다 하나씩 쌓였다. error/canceled 도 종결한다
- **결제 직후 배너 잔존** — 화면을 띄운 채 결제하면 initState 에서 만든 배너가
  그대로 남아 방금 결제한 유저에게 계속 보였다(주 결제 퍼널이 정확히 이 경로).
  세 화면(퀴즈·AI 모의고사·기출)에 복붙된 배너 블록을 프리미엄을 구독하는
  `widgets/banner_ad_bar.dart` 정본으로 통합
- 리워드 광고 show() 예외 시 다음 광고 미충전, 콜백 유실 시 호출자가 영영
  잠기던 것(3분 타임아웃 안전판). `AdService.dispose()` 에도 리워드 광고 해제를
  넣었지만 이 서비스는 앱 수명 싱글턴이라 실제로 불리는 곳이 없다 — 누수 수정으로
  치지 않는다

#### 검증
- 테스트 284건 → **304건** (`analyze` 0건). 신규: 리셋 쿼터·게이팅 경계값,
  error/canceled 종결, BannerAdBar 프리미엄 게이트, 배선 테스트 3종
- 쿼터는 기기 로컬 날짜 기준이라 날짜 조작·재설치로 우회 가능 — 서버 없이는
  못 막는 알려진 한계로 기록해 둔다

### v1.7.0+34 (2026-08-12)

**실제 복원 기출이 처음으로 앱에 들어왔다.** 지금까지 이 앱의 문항은 전부 AI 예상
문제였다. 2026년 2회 복원 기출 12문항을 추가하고, AI 문항과 섞이지 않도록 데이터
모델부터 화면까지 출처를 관통시켰다.

- `questions.source` 컬럼 추가 (**DB v8**) — `restored` / `ai`.
  기존 유저의 문항은 전부 `ai` 로 채워진다
- **마이그레이션 순서 주의** — v8 컬럼 추가는 v6 동기화보다 **먼저** 실행해야 한다.
  버전 순서대로 두면 v6 동기화의 INSERT 가 아직 없는 `source` 컬럼을 참조해
  v5 이하 유저의 업그레이드가 통째로 실패한다. 순서를 뒤집어 고쳤다
- 회차 목록이 `(연도, 회차, 출처)` 단위로 묶인다. 같은 "2026년 2회" 안에서도
  복원 기출 12문항과 AI 예상 41문항이 **별도 항목**으로 보인다
- 문제 카드 배지는 이제 문항의 `source` 만 본다 (연도로 추측 금지)
- 출처·라이선스 고지를 회차 목록 상단에 상설 노출
- 복원 기출은 **유료 잠금 없이 전체 공개** (광고는 앱 전체와 동일)
- 문제 데이터 에셋 목록을 `DatabaseService.questionAssetFiles` 정본으로 통일 —
  테스트가 목록을 따로 적어두는 바람에 새 파일 20문항이 검증 밖으로 샐 뻔했다

테스트 185건 → **186건**. `analyze` 0건.

> **2026년 2회 20문항 전부** 수록했다. 표가 이미지로 돼 있던 8문항(1·8·10·12·13·14·19·20)
> 은 이미지를 읽어 **텍스트 표로 옮겼다** — 앱에 이미지 표시 경로가 없기도 하고,
> 텍스트가 화면 크기·글씨 배율에 훨씬 잘 맞는다. CC BY 4.0 은 변형을 허용하며,
> 이 '변경함' 사실도 여기에 기록해 둔다.
