...
- 확인 방법: 글 HTML 소스에 `creativecommons.org/licenses/by/4.0/deed.ko` 링크와
  티스토리 CCL 마크업(`ccl-type`, `ccl-derive`)이 들어 있다. 화면의 배지는 작아서
  눈에 잘 안 띄므로 소스에서 확인하는 편이 확실하다.
- **티스토리 정책이 아니라 저작자가 글마다 직접 켠 설정이다.** 그래서 의미가 있다 —
  저작권자 본인이 "표시만 하면 상업적 이용·변형 모두 허용"이라고 명시한 것이다.
- CC BY 4.0 이 허용: 복제·배포, 변형·2차적 저작물, **상업적 이용**(광고·유료 앱 포함).
- CC BY 4.0 의 **유일한 의무: 저작자 표시.** 그래서 앱은 회차별 문제집 상단에
  `출처: Life-Journey 블로그 (CC BY 4.0)` 를 항상 노출한다.
  `test/round_list_screen_test.dart` 가 이 표기가 사라지는 것을 막는다.
