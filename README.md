...
- 광고 단위는 전부 실 단위다. 테스트 ID 복귀는 `test/ad_id_test.dart` 가 막는다
...
├── question_data_test.dart          # **1000문항 전수 채점** + 데이터 정합성
├── ad_id_test.dart                  # 광고 단위가 실 ID 인지 (수익 직결)
...
> ⚠️ **이 문항은 전부 AI 가 생성한 예상문제다. 기출 시험지가 아니다.**
> 연도·회차 필드는 "그 회차의 출제 경향을 본떴다" 는 뜻이지 "그 회차에 나온
> 문제" 가 아니다. 실제 정보처리기사 실기는 **회차당 20문항**인데 이 데이터는
> 회차당 40~60문항이라 애초에 기출 시험지일 수 없다.
>
> 한때 화면에서 2025년 이하를 '기출' · '기출 기반' 으로 표시했으나 사실과 달라
> 걷어냈다(v1.6.9). **어떤 화면·스토어 설명에서도 기출이라고 주장하지 말 것.**
> `test/round_list_screen_test.dart` 가 '기출' 표기 부활을 막는다.
...
단위로 배포되어 리워드 노출 수익이 0 이었다 — 앱은 정상 동작하고 응시권도
지급되니 화면·로그 어디에도 이상이 드러나지 않는, 조용히 돈이 새는 상태였다.

- AdMob 실 광고 단위로 교체 (Android `…/4234668134`, iOS `…/4862826277`).
  두 단위의 앱 ID 가 `AndroidManifest` / `Info.plist` 의 기존 앱 ID 와
  일치하는지 대조해 확인했다
- **`test/ad_id_test.dart` 신설** — 같은 사고가 다시 나지 않게 막는다.
  테스트 계정 단위 사용, 남의 퍼블리셔 계정, 지면 간 ID 중복을 전부 잡는다.
  iOS 만 테스트 ID 로 되돌리는 뮤테이션(과거 실제로 있었던 한쪽만 수정 패턴)이
  3건 실패하는 것을 확인하고 넣었다

테스트 180건 → **184건**. `analyze` 0건.

> 새 광고 단위는 노출이 시작되기까지 보통 1시간, 길게는 하루가 걸린다.
> 배포 직후 리워드가 안 뜨는 것은 정상이다.

### v1.6.4+28 (2026-08-12)

**1000문항 전수 채점 테스트를 도입하고, 그 테스트가 v1.6.2 의 자체 회귀를 잡아냈다.**

- **회귀 수정 (v1.6.2 가 만든 것)** — 숫자가 공백으로 구분된 정답에서 공백 제거
  비교를 통째로 건너뛰게 했더니, 실행 결과 정답에 흔한 군더더기 공백
  (`'1 2 3 \n2 3 4 '` — 값마다 뒤에 공백)까지 유저가 똑같이 적어야 정답이 됐다.
  노출 문항 21개. 이제 **숫자와 숫자 사이의 공백만 남기고** 나머지는 지운다 —
  "3050"(두 값을 하나로 오해한 오답)은 여전히 오답, 군더더기 공백은 흡수
- **전수 채점 테스트 신설** (`test/question_data_test.dart`) — 채점 규칙은 자주
  손대는데, 그때마다 "등록된 정답이 정답으로 채점되는가"를 확인할 방법이 없었다.
  예제 몇 개짜리 단위 테스트로는 1000문항 중 무엇이 깨졌는지 알 수 없다. 이제
  전 문항을 실제로 채점하고, 여러 줄 정답을 쉼표로 이어 쓴 답까지 검증한다.
  데이터 정합성(동기화 키 중복·빈 정답·미지의 유형)도 함께 본다

테스트 175건 → **180건**. `analyze` 0건.
...
#### 검증
- 테스트 160건 → **175건**. `analyze` 0건. 시뮬레이터 통합 테스트 통과
- 배선 테스트 강화: 리워드 다이얼로그 진입점, 홈 복귀 loadStats 2곳,
  홈 탭 갱신 조건, 기출/예상 경계, 복습 졸업, DB v6→v7 교체
- **purchaseStream 핸들러 배선 테스트 신설** — 실제 결제가 흐르는 유일한
  경로인데 지금까지 무방비였다(v1.5.4 결제 소실이 터졌던 지점). purchased/
  restored 콜백이 지급→저장→종결 순서로 이어지는 계약을 검증. 지급 제거
  뮤테이션이 실제로 실패함을 확인
- 통합 테스트 연타 시나리오 공회전 수정(#25) — 조용한 건너뛰기 가드를 제거하고
  입력창 대기 + "제출에 실제 도달했는가"(sessionSolved ≥ 1) 단언 추가

> **남은 것:**
> - ~~리워드 광고 실 ID 교체~~ → **v1.6.5 에서 완료**
> - PrivacyInfo `NSPrivacyTrackingDomains` 빈 배열 — 보류. Google Mobile Ads SDK
>   가 자체 매니페스트로 도메인을 선언해 실동작은 정상이고, 현재 ITMS 자동 검증도
>   막지 않는다. 심사에서 문의가 오면 그때 채운다.
> - DB v1 유저 학습 이력 삭제 — **의도된 손실로 확정**(코드에 사유 명시).
>   v1 은 이 저장소 첫 커밋보다 이전이라 스키마를 알 수 없어, 이력만 살리면
>   기록이 엉뚱한 문제에 붙는다. 대상자는 2026-04 이전 설치 후 미실행 유저뿐.
